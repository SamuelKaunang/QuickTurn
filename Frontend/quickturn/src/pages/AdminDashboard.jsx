...
import {
    Users, Megaphone, LogOut,
    Shield, Trash2, PlusCircle, Search,
    FileText, Activity, X, Flag, Ban, ShieldCheck
} from 'lucide-react';
...
const AdminDashboard = () => {
    ...
    const filteredUsers = users.filter(user => {
        const matchesSearch = user.nama?.toLowerCase().includes(userSearch.toLowerCase()) ||
            user.email?.toLowerCase().includes(userSearch.toLowerCase());
        const matchesRole = userRoleFilter === "ALL" || user.role === userRoleFilter;
        const matchesStatus = userStatusFilter === "ALL" ||
            (userStatusFilter === "ACTIVE" && user.enabled) ||
            (userStatusFilter === "BANNED" && !user.enabled);
        return matchesSearch && matchesRole && matchesStatus;
    });

    const filteredProjects = projects.filter(project => {
        const matchesSearch = project.title?.toLowerCase().includes(projectSearch.toLowerCase()) ||
            project.owner?.nama?.toLowerCase().includes(projectSearch.toLowerCase());
    ...
    const handleToggleBan = async (userId, isCurrentlyActive) => {
        ...
        try {
            ...
            if (res.ok) {
                const data = await res.json();
                alert(data.message || `User ${action}ned successfully`);
                fetchUsers(token);
            } else {
                ...
                <section className="content-body" style={{ paddingTop: '2rem' }}>
                        ...
                        <div className="fade-in">
                            <GlassCard className="p-6">
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
                                    <h3 className="text-xl font-bold" style={{ color: 'var(--slate-900)', margin: 0 }}>All Users ({filteredUsers.length})</h3>
                                            ...
                                            {filteredUsers.length === 0 ? (
                                                ...
                                                filteredUsers.map(u => (
                                                    <tr key={u.id} style={{ borderBottom: '1px solid var(--slate-200)' }}>
                                                        ...
                                                        <td className="p-3"><span className={`badge ${u.role === 'MAHASISWA' ? 'bg-blue-500/20 text-blue-800' : u.role === 'ADMIN' ? 'bg-purple-500/20 text-purple-800' : 'bg-green-500/20 text-green-800'} px-2 py-1 rounded text-xs`}>{u.role === 'MAHASISWA' ? 'Talent' : u.role === 'UMKM' ? 'Client' : u.role}</span></td>
                                                        <td className="p-3">
                                                            <span style={{
                                                                padding: '4px 10px',
                                                                borderRadius: '20px',
                                                        ...
                                                        <td className="p-3">
                                                            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                                                                ...
                                                                {u.role !== 'ADMIN' && (
                                                                    <button
                                                                        onClick={() => handleToggleBan(u.id, u.enabled)}
                                                                        className={u.enabled ? "text-orange-500 hover:text-orange-700 p-2" : "text-green-500 hover:text-green-700 p-2"}
                                                                        title={u.enabled ? "Ban User" : "Unban User"}
                                                                        style={{
                                                                            background: 'none',
                                                                            border: 'none',
                                                                            cursor: 'pointer',
                                                                    ...
                                                                    >
                                                                        {u.enabled ? <Ban size={16} /> : <ShieldCheck size={16} />}
                                                                    </button>
                                                                )}
                                                                <button
                    ...
                    {activeTab === 'projects' && (
                        <div className="fade-in">
                            <GlassCard className="p-6">
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
                                    <h3 className="text-xl font-bold" style={{ color: 'var(--slate-900)', margin: 0 }}>All Projects ({filteredProjects.length})</h3>
                                                ...
                                                filteredProjects.map(p => (
                                                    <tr key={p.id} style={{ borderBottom: '1px solid var(--slate-200)' }}>
                                                        ...
                                                        <td
                                                            ...
                                                            onClick={() => p.owner?.id && navigate(`/profile/${p.owner.id}`)}
                                                        >
                                                            {p.owner ? p.owner.nama : 'Unknown'}
                                                        </td>
                                                        <td className="p-3">
                                                            <span style={{
                                                                padding: '4px 10px',
                                                                ...
                                                                fontSize: '0.75rem',
                                                                fontWeight: '600',
                                                                background: p.status === 'OPEN' ? 'rgba(34, 197, 94, 0.1)' :
                                                                    p.status === 'IN_PROGRESS' ? 'rgba(59, 130, 246, 0.1)' :
                                                                        p.status === 'COMPLETED' ? 'rgba(124, 58, 237, 0.1)' : 'rgba(239, 68, 68, 0.1)',
                                                                color: p.status === 'OPEN' ? '#16a34a' :
                                                                    p.status === 'IN_PROGRESS' ? '#2563eb' :
                                                                        p.status === 'COMPLETED' ? '#7c3aed' : '#dc2626'
                                                            }}>
                                                                {p.status}
                                                            </span>
                                                        </td>
