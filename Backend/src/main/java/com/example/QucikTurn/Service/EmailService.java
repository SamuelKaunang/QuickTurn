...
public class EmailService {
    ...
    /**
     * Send verification code email for email verification.
     * Uses @Async to avoid blocking the main thread.
     */
    @Async
    public void sendEmailVerificationCode(String toEmail, String userName, String code) {
                                ...
                                <table role="presentation" width="600" cellspacing="0" cellpadding="0" border="0" style="background-color: rgba(255, 255, 255, 0.95); border-radius: 16px; box-shadow: 0 4px 24px rgba(0, 0, 0, 0.08), 0 10px 40px rgba(0, 0, 0, 0.04); overflow: hidden; border: 1px solid rgba(255, 255, 255, 0.8);">
                                    ...
                                    <tr>
                                        <td style="padding: 40px; background-color: #ffffff;">
                                            ...
                                            <p style="color: #475569; font-size: 15px; line-height: 1.7; margin: 0 0 30px 0;">
                                                Welcome to QuickTurn! To complete your registration and activate your account, please enter the verification code below in the app:
                                            </p>
                                            ...
                                            <div style="background-color: #fff1f2; border-left: 4px solid #e11d48; border-radius: 0 8px 8px 0; padding: 16px 20px; margin: 30px 0;">
                                                <p style="color: #9f1239; font-size: 14px; margin: 0; line-height: 1.5;">
                                                    <strong>⏱ Important:</strong> <span style="color: #64748b;">This verification code will expire in <strong style="color: #0f172a;">15 minutes</strong>.</span>
                                                </p>
                                            </div>

                                            <p style="color: #64748b; font-size: 14px; line-height: 1.6; margin: 30px 0 0 0;">
                                                If you did not register for a QuickTurn account, please ignore this email.
                                            </p>
                                        </td>
                                    </tr>
        ...
        try {
            ...
            CreateEmailResponse response = resend.emails().send(createEmailOptions);
            log.info("Email verification code sent successfully to: {} (ID: {})", maskEmail(toEmail), response.getId());

        } catch (ResendException e) {
            log.error("Failed to send email verification code to: {} - {}", maskEmail(toEmail), e.getMessage());
        }
    }
